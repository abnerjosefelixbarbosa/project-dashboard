...
public record LoginAccountRequest(
		...
		@NotNull(message = "password user null") @Length(min = 1, max = 20, message = "passoword user at min 1 and max 20") @Pattern(regexp = "^(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).+$", message = "password user invalid") String passwordUser) {
}